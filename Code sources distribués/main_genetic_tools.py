...
class Genome_tools (object) :
    ...
    def __init__(self, agent):
        self.list_population = [(fitness, parametre) for fitness, parametre in agent.format_reservoir()]
        # Nombre d'agents dans la population :
        self.nbAgents = len(self.list_population)
    ...
    def algorithme_genetique_eliste (self, str_selection, size_tounois, str_mutation, taux_mutation, sigma) : 
...
#        print("parent\n", parents_selectionnes)

        # Creation d'enfant à partir de mutation d'enfant :
        if str_mutation == 'mutation_gaussienne' :
            enfant_genere = self.mutation_gaussienne(parents_selectionnes , sigma, taux_mutation, nb_enfants_par_parent) # Attention taux de mutation à 1.
        else :
            enfant_genere = self.mutation_uniforme_limite(parents_selectionnes , sigma, nb_enfants_par_parent)
